...
class HSRnet(nn.Module):
    def __init__(self,in_channels,out_channel):
        ...
        self.conv_B = self.convlayer(2*in_channels-1, 6, 3)
        self.conv_G = self.convlayer(2*in_channels-1, 10, 3)
        self.conv_R = self.convlayer(2*in_channels-1, 15, 3)
...
def spectral_grad(x):
    bs,c,h,w=x.shape
    y=torch.zeros(bs,2*c-1,h,w).cuda()
    for i in range(x.shape[1]-1):
        y[:, 2 * i, :, :] = x[:, i, :, :]
        y[:,2*i+1,:,:]=abs(x[:,i+1,:,:]-x[:,i,:,:])
    y[:, -1, :, :] = x[:, -1, :, :]
    return y
