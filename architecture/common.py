...
from architecture.modules import InvertibleConv1x1
...
class ResidualGroup(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, n_resblocks):
        ...
        modules_body = [
            RCAB(
                conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.LeakyReLU(negative_slope=0.2, inplace=False), res_scale=1) \
    ...
    for net in net_l:
        for m in net.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data = scale* m.weight.data# for residual block
...
class Get_gradient_nopadding_d(nn.Module):
    def __init__(self,num_channels=31):
...
#         super(Get_gradient_nopadding_d, self).__init__()
#         self.num_channels = num_channels
#         self.weight_v = nn.Parameter(torch.zeros(1, 1, 3, 3), requires_grad=False)
#         self.weight_h = nn.Parameter(torch.zeros(1, 1, 3, 3), requires_grad=False)
...
#             x_i = x[:, i]
#             x_v = F.conv2d(x_i.unsqueeze(1), self.weight_v, padding=1)
#             x_h = F.conv2d(x_i.unsqueeze(1), self.weight_h, padding=1)
#             x_grad[:, i:i+1, :, :] = torch.sqrt(torch.pow(x_v, 2) + torch.pow(x_h, 2) + 1e-6)
#         return x_grad
##########################################################
class GCM(nn.Module):
    def __init__(self,n_feats,scale):
        ...
        self.grad_d = Get_gradient_nopadding_d(n_feats)
        self.upBlock = DenseProjection(n_feats, n_feats, scale, up=True, bottleneck=False)
        self.downBlock = DenseProjection(n_feats, n_feats, scale, up=False, bottleneck=False)
        self.c_rgb = default_conv(3,n_feats,3)
        self.c_d = default_conv(n_feats,n_feats,3)
        ...
        self.rb_rgbd = ResBlock(default_conv, n_feats, 3, bias=True, bn=False,
                                act=nn.LeakyReLU(negative_slope=0.2, inplace=False), res_scale=1)
        self.fuse_process = nn.Sequential(nn.Conv2d(2*n_feats, n_feats, 1, 1, 0),
                                          ...
                                          ResidualGroup(default_conv, n_feats, 3, reduction=8, n_resblocks=4))
        self.re_g = default_conv(n_feats,n_feats,3)
        self.re_d = default_conv(n_feats,1,3)
        self.c_sab = default_conv(1,n_feats,3)
        self.sig = nn.Sigmoid()
        self.d1 = nn.Sequential(default_conv(n_feats,n_feats,3),
